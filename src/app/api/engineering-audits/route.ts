...
const bodySchema = z.object({
  ...
  safety: booleanField,
  rootCause: booleanField,
  materialPass: booleanField,
  testingPass: booleanField,
...
export async function POST(request: Request) {
  const auth = requireRole(request, [UserRole.TECHNICIAN]);
  ...
  try {
    ...
    if (!checksComplete) {
      return NextResponse.json({ error: 'Checklist ต้องครบทุกข้อ' }, { status: 400 });
    }

    const result = await prisma.$transaction(async (tx) => {
      const booking = await tx.booking.findUnique({ where: { id: data.bookingId }, include: { payment: true } });
      if (!booking || booking.technicianId !== data.technicianId) {
        throw new Error('ไม่พบงานหรือช่างไม่มีสิทธิ์ส่งงานนี้');
      }
      if (!booking.payment || booking.payment.status !== 'HOLD') {
        throw new Error('รายการเงินไม่อยู่ในสถานะ HOLD');
      }
      if (['COMPLETED', 'CANCELLED', 'DISPUTED'].includes(booking.status)) {
        throw new Error('งานนี้ไม่สามารถส่ง QC ซ้ำได้');
      }
      ...
      const checklist = await tx.checklist.create({
        data: {
          ...
          status: 'SUBMITTED',
          safetyChecked: data.safety,
          rootCauseIdentified: data.rootCause,
          materialStandardPassed: data.materialPass,
          leakPressureTested: data.testingPass,
          siteCleaned: data.cleaned,
          beforePhotoUrl,
          afterPhotoUrl,
