...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!Number.isInteger(id)) return NextResponse.json({ error: 'bookingId ไม่ถูกต้อง' }, { status: 400 });

    const result = await prisma.$transaction(async (tx) => {
      const booking = await tx.booking.findUnique({ where: { id }, include: { payment: true, checklists: { orderBy: { submittedAt: 'desc' }, take: 1 } } });
      ...
      if (!booking.checklists[0]) throw new Error('ไม่พ���รายงาน QC');
